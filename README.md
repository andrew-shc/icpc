...
| 2222C               | ❌                         | solution per discord; very elegant DP problem; idea: dp[i] is the max valid partitioning available and we do a 2nd loop backwards to compute dp[j-1]+1 (where j to i is 1 valid group) and check against dp[i] if its greater or not                |
| 1973C               | ❌                         | some missing insight such as if the largest (n) is on even/odd, then that set of indices should contain all larger, and summing it with top-half of another permutation guarantee >=n+1 (if we assign the smallest with the largest)                |
| 1969C               | ❌  (yea i'm lwk pathetic) | (finally more DP exposure) another elegant DP problem: key idea is the inductive step assumes given d ops, we can have all d+1 adjacent elements assigned to the min and iterate d O(k) within the DP framework O(nk) resulting O(nk^2)             |
| 1957C               | ❌                         | combinatorically: observe that yes we care about the ordering within each pair, but we do not care about the ordering between other pairs in terms of pairs (or by pairs); DP-able: key obs., we can pair with *any* i-1 rows/cols (key invariance) |
| 1956C               | ❌                         | 1600 *constructive* problem => just get better                                                                                                                                                                                                      |
